...
Ripgrep-based search and refactoring tool
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Prettify symbols
Displays fncy characters/symbols
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** TRAMP
- To access other devices on the local network, run all TRAMP traffic through kubotunnel. For instance, this lets you access rpi frame
#+BEGIN_SRC emacs-lisp
  (add-to-list 'tramp-default-proxies-alist
               '("192.168.1.24" nil "/ssh:kubo@ryanwcummings.com#443:"))
#+END_SRC

- Force tramp to use bash, because it doesn't like zsh
#+BEGIN_SRC emacs-lisp
...
Set up auto-venv activation. To use, name the venv the same name as the project root directory
#+BEGIN_SRC emacs-lisp
...
(use-package org
  ...
  (org-capture-templates
   (quote (("t" "Todo" entry (file "~/org/inbox.org")
           ...
           ("j" "Journal" entry (file+olp+datetree "~/org/journal.org")
            "* %?\n%U\n")
           ("p" "Programming Log" entry (file+headline "~/org/notebook.org" "Programming Log")
            "* %u %?\n")
           )))
  (org-log-done 'time)
  ...
  (org-agenda-files
   (quote
    ("~/org/inbox.org" "~/org/gtd.org" "~/org/schedule.org" "~/org/habits.org")))
